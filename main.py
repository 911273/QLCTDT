...
from sections.sec5_hoc_lieu  import Sec5HocLieu
...
class QLCTDTApp:
    ...
    def _build_menu(self):
        ...
        mfile.add_separator()
        mfile.add_command(label='✖ Thoát',                   command=self.root.quit)
    ...
    def _build_body(self):
        ...
        self.sec3 = Sec3MucTieu(self.nb, self.db, lazy=True, modified_callback=_on_modified)
        self.sec4 = Sec4Clo(self.nb, self.db, lazy=True, modified_callback=_on_modified)
        self.sec5 = Sec5HocLieu(self.nb, self.db, lazy=True, modified_callback=_on_modified)
    ...
    def _on_hp_select(self, _e=None):
        ...
        if self._modified:
            ans = messagebox.askyesnocancel('Chưa lưu',
                'Học phần hiện tại chưa được lưu.\nLưu trước khi chuyển không?')
            if ans is None:
                return
            if ans:
                self.save_hp()
    ...
    def save_hp(self):
        ...
        success = self.controller.save_current_hp(self.current_hp_id, full_data, modified_data)
    ...
    def export_word_builtin(self):
        ...
        self.save_hp()
        hp = self.db.get_hoc_phan(self.current_hp_id)
        default_name = (hp['ten_viet'] or 'DecuongHP').replace(' ', '_') + '.docx'
        ...
        if self.controller.export_word(self.current_hp_id, out_path, mode='builtin'):
            if self.ask_yesno('Xuất thành công', f'Đã xuất:\n{out_path}\n\nMở file ngay không?'):
    ...
    def open_template_manager(self):
        _TemplateManagerDialog(self.root, self.db)
