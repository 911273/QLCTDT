...
class DeCuongSection(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        tb.Button(self.toolbar, text="⚙ Tùy biến UI", bootstyle=WARNING,
                  command=self._toggle_inline_crud).pack(side=LEFT, padx=5)
    ...
    def _toggle_inline_crud(self):
        """Bật/tắt chế độ Inline CRUD."""
        if hasattr(self, '_overlay_window') and self._overlay_window and self._overlay_window.winfo_exists():
        ...
        # Chỉ áp dụng overlay cho tab hiện tại
        current_tab_idx = self.nb.index("current")
        # Lấy widget con của tab hiện tại, ví dụ tab1, tab2...
        current_tab_widget = None
        if current_tab_idx == 0: current_tab_widget = self.tab1
        elif current_tab_idx == 1: current_tab_widget = self.tab2
        elif current_tab_idx == 2: current_tab_widget = self.tab3
        # ... Do deccuong_section.py là file cũ và các sec*.py là bản refactor
        # Nên nếu áp dụng Overlay ở đây, overlay cần truyền vào section_widget tương ứng.
        # Ở đây ta sẽ giả định self là container chính và Overlay sẽ tìm các frame.
        self._overlay_window = InlineCRUDOverlay(self, self.db, self)
